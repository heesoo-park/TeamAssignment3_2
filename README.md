...
### 만든 기능 :
#### 1. 로그인 페이지
1) 아이디는 회원가입하고 돌아왔을 때 채워져있음
2) 비밀번호는 보안을 유지하기 위해 ***로 표시
3) 로그인 버튼을 클릭하면 메인 페이지로 이동하거나 또는 오류 메세지 출력
4) 회원가입 버튼을 클릭하면 회원가입 페이지로 이동

#### 2. 회원가입 페이지
1) 아이디 중복 체크 기능 구현
2) 비밀번호 유효성 검사
3) 비밀번호는 보안을 유지하기 위해 ***로 표시
4) 다음 버튼은 모든 조건(입력칸이 비어있지 않고 각각의 카테고리 검사를 통과)을 만족 시 활성화
5) 버튼 클릭시 프로필 이미지 선택 페이지로 이동

#### 3. 프로필 이미지 선택 페이지
1) 하나의 이미지만 선택 가능
2) 미리보기 화면에서 선택한 이미지 노출
3) 회원가입 버튼은 이미지를 선택하면 활성화
4) 버튼 클릭시 로그인 페이지로 되돌아감
5) 편집 페이지에서 넘어온 경우, 회원가입 버튼의 텍스트가 수정하기로 변경

#### 4. 메인 페이지
1) 스크롤뷰 사용하여 게시물 리스트와 사용자 리스트가 스크롤 가능
2) 게시물 있는 영역만 스크롤 됨
3) 이미지 또는 하트를 누르면 좋아요 숫자 증가 & 좋아요 이미지 변경
4) 게시물의 양 옆의 화살표 버튼을 누름으로써 게시물 이미지 전환
5) 양 옆의 화살표 버튼은 이미지의 인덱스에 따라 화면 표시 여부가 달라짐
6) 사용자 프로필 이미지를 누르면 디테일 페이지(or 마이 페이지)로 이동
7) 사용자 프로필 이미지에서 로그인한 유저만 다른 테두리 색을 가짐
8) 게시물 리스트 영역만 세로 스크롤 됨
9) 사용자 리스트 영역만 가로 스크롤 됨
10) 더보기를 누르면 잘려있던 게시글이 전부 보임
11) 인사말 속 이름이 사용자에 따라 변하도록 설정
12) 게시물은 layoutInflater를 이용하여 생성

#### 5. 디테일 페이지
1) 스크롤뷰 사용하여 게시물 리스트가 스크롤 가능
2) 게시물의 양 옆의 화살표 버튼을 누름으로써 게시물 이미지 전환
3) 양 옆의 화살표 버튼은 이미지의 인덱스에 따라 화면 표시 여부가 달라짐
4) 이미지 또는 하트를 누르면 좋아요 숫자 증가 & 좋아요 이미지 변경
5) 게시물 리스트 영역만 세로 스크롤 됨
6) 더보기를 누르면 잘려있던 게시글이 전부 보임
7) 댓글이 있는 구역을 누르면 댓글 페이지로 이동
8) 디테일 페이지로 사용될 때는 편집, 로그아웃 버튼이 보이지 않음
9) 게시물은 layoutInflater를 이용하여 생성

#### 6. 마이 페이지
1) 디테일 페이지를 재활용하여 구현
2) 편집 버튼을 누르면 편집 페이지로 이동
3) 로그아웃 버튼을 누르면 로그아웃되어 로그인 페이지로 이동
4) 편집 기능을 통해 회원정보(이름, 상태 메시지)가 수정됐다면 바로 반영
5) 스크롤뷰 사용하여 게시물 리스트가 스크롤 가능
6) 게시물 리스트 영역만 세로 스크롤 됨
7) 게시물의 양 옆의 화살표 버튼을 누름으로써 게시물 이미지 전환
8) 양 옆의 화살표 버튼은 이미지의 인덱스에 따라 화면 표시 여부가 달라짐
9) 이미지 또는 하트를 누르면 좋아요 숫자 증가 & 좋아요 이미지 변경
10) 더보기를 누르면 잘려있던 게시글이 전부 보임
11) 댓글이 있는 구역을 누르면 댓글 페이지로 이동
12) 게시물은 layoutInflater를 이용하여 생성

#### 7. 편집 페이지
1) 회원가입 페이지를 재활용하여 구현
2) 아이디 입력칸을 상태 메시지칸으로 변경
3) 비밀번호는 보안을 유지하기 위해 ***로 표시
4) 다음 버튼은 모든 조건(입력칸이 비어있지 않고 각각의 카테고리 검사를 통과)을 만족 시 활성화
5) 버튼 클릭시 프로필 이미지 선택 페이지로 이동

#### 8. 게시물 페이지
1) 게시물 각각의 아이디를 줘서 게시물이 구별되게 함
2) 스크롤뷰 사용하여 게시물 리스트가 스크롤 가능
3) 이미지 또는 하트를 누르면 좋아요 숫자 증가 & 좋아요 이미지 변경
4) 댓글 작성 기능 
5) 댓글 저장 기능

#### 9. 다크 모드
1) res/values-night 디렉토리 활용

#### 10. 가로 모드
1) res/values-land 디렉토리 활용
2) 레이아웃이 한눈에 들어오지 않는 경우가 있어서 ScrollView를 사용

#### 11. 영어 버전 모드
1) res/values-en 디렉토리 활용

#### 12. 원형 이미지뷰
1) CardView 활용

#### 13. Activity 전환 시 animation 구현
1) overridePendingTransition() 사용
2) 액티비티 시작과 끝에 적용

#### 14. 회원정보 관리 구현
1) object class로 구현
2) 더미 데이터 추가
3) 필요한 함수들 추가(add, get, edit)
